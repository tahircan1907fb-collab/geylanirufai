...
export default function EventsPage() {
     ...
     async function handleSave(e: React.FormEvent) {
          ...
          if (!form.title || !form.date || !form.time || !form.location) {
               adminToast('Lütfen tüm zorunlu alanları doldurunuz.', 'warning');
               return;
          }
          ...
          try {
               ...
               adminToast(editing ? 'Etkinlik güncellendi' : 'Yeni etkinlik eklendi', 'success');
               setModalOpen(false);
               fetchAll();
          } catch (error) {
               console.error('Save error:', error);
               adminToast('Kayıt sırasında hata oluştu', 'error');
          } finally {
               setSaving(false);
          }
     }
     ...
     async function handleDelete(id: number) {
          ...
          try {
               ...
               adminToast('Etkinlik silindi', 'success');
               fetchAll();
          } catch (error) {
          ...
          <div>
               ...
               <div className="flex gap-3 mb-4">
                    <div className="flex-1 relative">
                         ...
                         <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Etkinlik ara..." className="w-full pl-10 pr-4 py-2.5 bg-slate-800 border border-slate-700 rounded-lg text-white text-sm placeholder-slate-500 focus:outline-none focus:border-amber-500 transition" />
                    </div>
                    <select value={filterCat} onChange={(e) => setFilterCat(e.target.value)} className="px-3 py-2.5 bg-slate-800 border border-slate-700 rounded-lg text-white text-sm focus:outline-none focus:border-amber-500 transition">
