...
export default function AboutSettings() {
     ...
     useEffect(() => {
          fetch('/api/settings', { headers: authHeaders() })
               ...
               .catch((err) => console.error('AboutSettings fetch error:', err))
     ...
     return (
          <form onSubmit={handleSave} className="max-w-4xl mx-auto">
               <div className="flex items-center justify-between mb-6">
                ...
                <section className="mt-6 bg-slate-900 border border-slate-700 rounded-xl p-5">
                     <h2 className="text-white font-semibold mb-3">Mevcut Veri Özeti</h2>
                     <ul className="text-sm text-slate-300 space-y-1">
                          <li>Başlık: {form.aboutTitle || '-'}</li>
                          <li>Paragraf 1: {(form.aboutText1 || '-').slice(0, 120)}</li>
                          <li>Paragraf 2: {(form.aboutText2 || '-').slice(0, 120)}</li>
                          <li>Alıntı: {form.aboutQuote || '-'}</li>
                     </ul>
                </section>
           </form>
     );
